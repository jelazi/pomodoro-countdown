...
class _AddProjectState extends State<AddProject> {
  ...
  addProject() {
    ...
    if (durationMinutes > 45) {
      ...
      MySnackBar.warningSnackBar('tooSmall'.tr, 'This duration is too short.');
      return;
    }
    String nameProject = name;
    if (_projectsController.existsNameProject(nameProject)) {
      MySnackBar.warningSnackBar('sameName'.tr, 'There is same name project.');
      return;
    }
    _projectsController.newProject(
        nameProject, _settingsController.owner?.name ?? '');
    Navigator.of(Get.overlayContext!).pop();
    MySnackBar.warningSnackBar(
        'newProject'.tr, 'Project $nameProject  created.');
  }
